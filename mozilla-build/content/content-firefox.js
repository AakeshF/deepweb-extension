...
(function() {
  ...
  function createChatUI() {
    try {
      ...
      // Use innerHTML cautiously - this is safe because it's our own static template
      // No user input is interpolated into this HTML
      container.innerHTML = `
        <div style="
          display: flex;
          align-items: center;
...
">Send</button>
      ...
      if (document.body) {
        ...
        console.error('[DeepWeb] Document body not available');
        return;
      }
  ...
  function safeAddEventListener(element, event, handler, options = {}) {
    if (!element || typeof handler !== 'function') {
      console.error('[DeepWeb] Invalid element or handler for event:', event);
      return null;
    }
  ...
  function setupEventHandlers() {
    ...
    // Button hover effects using event delegation
    ...
    // Add hover listeners to the header container for event delegation
    const headerDiv = container.querySelector('div:first-child');
    if (headerDiv) {
      safeAddEventListener(headerDiv, 'mouseover', buttonHoverHandler);
      safeAddEventListener(headerDiv, 'mouseout', buttonHoverHandler);
    }
    ...
    // Event listeners
    safeAddEventListener(sendBtn, 'click', sendMessage);
  ...
  function addMessage(text, type, id) {
    ...
    // Use textContent for safe text insertion instead of innerHTML
  ...
  window.__deepwebCleanup = function() {
    console.log('[DeepWeb] Cleaning up event listeners...');
    eventCleanupFunctions.forEach(cleanup => {
      try {
        cleanup();
      } catch (error) {
        console.error('[DeepWeb] Error during cleanup:', error);
      }
    });
